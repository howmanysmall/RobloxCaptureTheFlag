...
function Data:SaveAll()
	...
	local promises = {}
	local length = 0
	for key in next, self._cache do
		length += 1
		promises[length] = self:Save(key)
	end
...
function Data:Start()
	...
	if game.GameId == 0 then
		self.IsUsingMockService = true
	elseif RunService:IsStudio() then
		if not self.SaveInStudio then
			self.IsUsingMockService = true
		else
	...
	local function AutoSaveAllData()
		...
		autoSaving = true
		local promises = {}
		for _, data in next, dataPool do
