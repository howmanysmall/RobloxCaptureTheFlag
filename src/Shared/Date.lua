...
function Date:Format(str)
	...
	str = str
		...
		:gsub("%%d", string.format("%.2i", d.day))
		:gsub("%%D", string.format("%.2i/%.2i/%s", d.month, d.day, string.sub(tostring(d.year), -2)))
		:gsub("%%F", string.format("%i-%.2i-%.2i", d.year, d.month, d.day))
		:gsub("%%H", string.format("%.2i", d.hour))
		:gsub("%%k", string.format("%.2i", d.hour))
		:gsub("%%I", string.format("%.2i", h12))
		:gsub("%%l", string.format("%.2i", h12))
		:gsub("%%j", string.format("%.3i", d.yday))
		:gsub("%%m", string.format("%.2i", d.month))
		:gsub("%%M", string.format("%.2i", d.min))
		:gsub("%%n", "\n")
		:gsub("%%p", d.hour >= 12 and "PM" or "AM")
		:gsub("%%P", d.hour >= 12 and "pm" or "am")
		:gsub("%%r", string.format("%.2i:%.2i:%.2i %s", h12, d.min, d.sec, d.hour >= 12 and "PM" or "AM"))
		:gsub("%%R", string.format("%.2i:%.2i", d.hour, d.min))
		:gsub("%%s", math.floor(self._s))
		:gsub("%%S", string.format("%.2i", d.sec))
		:gsub("%%t", "\t")
		:gsub("%%T", string.format("%.2i:%.2i:%.2i", d.hour, d.min, d.sec))
		:gsub("%%w", string.format("%.2i", d.wday))
		:gsub("%%y", string.sub(tostring(d.year), -2))
